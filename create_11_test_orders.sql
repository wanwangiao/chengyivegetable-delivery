...
-- 首先清理可能存在的測試訂單
DELETE FROM orders WHERE contact_name LIKE '測試客戶%';

-- 創建三峽區訂單 (4筆)
INSERT INTO orders (contact_name, contact_phone, address, status, driver_id, total_amount, delivery_fee, payment_method, created_at) VALUES
...
SELECT 
    ...
    30
FROM orders o 
WHERE o.contact_name LIKE '測試客戶%' AND o.id NOT IN (SELECT DISTINCT order_id FROM order_items WHERE order_id = o.id);
...
SELECT 
    ...
    25
FROM orders o 
WHERE o.contact_name LIKE '測試客戶%' AND o.id NOT IN (SELECT DISTINCT order_id FROM order_items WHERE order_id = o.id AND product_id = 2);
...
SELECT 
    ...
    COUNT(*) as count
FROM orders 
WHERE contact_name LIKE '測試客戶%' AND status = 'packed' AND driver_id IS NULL
GROUP BY 1
ORDER BY count DESC;
