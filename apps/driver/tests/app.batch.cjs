...
const run = async () => {
  try {
    ...
    const textContent = JSON.stringify(snapshot);
    if (!textContent.includes('建議批次')) {
      throw new Error('未找到建議批次標題');
    }
    if (!textContent.includes('預估距離')) {
      throw new Error('未找到預估距離說明');
    }
    if (!textContent.includes('領取整批')) {
      throw new Error('未找到領取整批按鈕');
    }

    tree.unmount();
    console.log('✅ Driver dashboard 推薦批次 測試通過');
    return true;
  } finally {
    cleanup();
  }
