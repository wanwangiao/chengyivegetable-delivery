...
export const createDriverRouter = (
  ...
  // Driver orders routes (preferred paths for frontend)
  router.get('/orders/available', authenticate(['DRIVER']), controller.listAvailableOrders);
  router.get('/orders/active', authenticate(['DRIVER']), ordersController.listActiveOrders);
  ...
  router.post('/orders/reorder', authenticate(['DRIVER']), ordersController.reorder);
