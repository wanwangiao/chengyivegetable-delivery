        ...
        if (demoMode) {
            ...
            const query = `
                SELECT o.*, 
                       ...
                       array_agg(
                           json_build_object(
                               'product_name', COALESCE(p.name, oi.product_name, '商品'),
...
router.get('/stats', async (req, res) => {
    try {
        ...
        if (demoMode) {
            ...
            const query = `
                SELECT 
                    COUNT(CASE WHEN status = 'delivered' AND DATE(created_at) = CURRENT_DATE THEN 1 END) as today_completed,
                    COALESCE(SUM(CASE WHEN status = 'delivered' AND DATE(created_at) = CURRENT_DATE THEN 50 END), 0) as today_earnings,
