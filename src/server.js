...
async function createDatabasePool() {
  ...
  // 檢測是否在 Railway 生產環境中 (檢查是否能夠訪問內部網址)
  const isRailwayProduction = process.env.RAILWAY_ENVIRONMENT === 'production' ||
                              (process.env.NODE_ENV === 'production' && process.env.RAILWAY_PROJECT_ID);

  // Railway 生產環境使用內部網路，其他環境使用外部網路
  const databaseUrl = isRailwayProduction ? railwayInternalUrl : railwayExternalUrl;

  console.log(`🚂 使用 ${isRailwayProduction ? 'Railway 生產環境 (內部)' : '外部'} 資料庫連線`);
  console.log(`🔗 連線字串: ${databaseUrl.replace(/:[^:@]*@/, ':***@')}`);

  try {
    // SSL 設定：Railway 內部網路不需要SSL，外部連線需要SSL
    const sslConfig = isRailwayProduction ? false : { rejectUnauthorized: false };
...
// 調試路由日誌
app.use('/api/orders/search', (req, res, next) => {
  console.log('🔍 接收到搜尋請求:', req.method, req.originalUrl);
  console.log('📍 即將調用next()');
  next();
  console.log('📍 next()已調用');
});
...
// 📱 即時訂單追蹤頁面
console.log('🔧 註冊路由: /track-order/:id');
app.get('/track-order/:id', asyncWrapper(async (req, res, next) => {
  try {
    const orderId = parseInt(req.params.id);
...
console.log('🔧 註冊路由: /api/orders/search/:phone');
...
// 獲取訂單狀態API (供前端使用)
app.get('/api/orders/:id/status', asyncWrapper(async (req, res) => {
  try {
    console.log('🔍 訂單狀態API被調用，ID參數:', req.params.id);
    const orderId = parseInt(req.params.id);
...
app.get('/api/orders/:id/details/:phone', asyncWrapper(async (req, res) => {
  try {
    const orderId = parseInt(req.params.id);
    const phone = req.params.phone;
